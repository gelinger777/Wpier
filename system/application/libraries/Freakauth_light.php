...
class Freakauth_light
{
  ...
  /**
	...
	function recaptcha_init($action)
    {	
    	//checks FreakAuth security code configuration
        if (!$this->CI->config->item('FAL_use_captcha'.$action))
		...
		$this->CI->config->load('freakauth_light');
		return $this->CI->recaptcha->get_html();
     ...
     * and edit custom user profile.
  	...
  	function _buildUserProfileFieldsRules()
  	{
		...
		//variable to hold the form_validation rules
		$config = array();
		
		//variable to hold the field names for the form
		$data = array();
  		// I use 'for' instead of 'foreach' because I have to escape the
        // 'id' field that has key=0 in my array
	  	for ($i=1; $i<=$num_db_fields;  $i++)
		{
			//create the individual rule for each field
			$sub_rule = array();
		
			$field = $db_fields[$i];
			 
			 //set the field name for the rule
			$sub_rule['field'] = $field;
			 ...
			 $sub_rule['rules'] = array_key_exists($field, $field_rule) ? $field_rule[$field] : '';
			 // creates fields
			 ...
			 $sub_rule['label'] =  array_key_exists($field, $field_name) ? $field_name[$field] : $field;
			 
			 //set the label name for the form
			 $data[$field] = $sub_rule['label'];
			 
			 array_push($config, $sub_rule);
		}

		//data contains the label names, config contains the rules
		$returnValue = array($data,$config);
		return $returnValue;
  	}
