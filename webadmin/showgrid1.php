...
<head>
  ...
  foreach($f_array as $k=>$v) if($v!='*hide*') {
...
<?}?>
  ...
  function ExportTab(x) {
     ...
     store.loadXls="/tmp/<?=$ADMIN_ID."_".$EXT."_".mktime()?>."+x;
    ...
    store = new Ext.data.<?=(isset($PROPERTIES["grouping"])? "GroupingStore":"Store")?>({
	  ...
	  load:function(x,y,z) {
  	    if(x.loadXls!=null && x.loadXls!='') {
			  ...
			  if(s=='.xls') window.location="/webadmin/dwlxsl.php?f="+x.loadXls; 
			  else {
...
<?}?>
	...
	MainGrid = new xg.<?=(($PERMIS[2] && isset($PROPERTIES["egrid"]) && $PROPERTIES["egrid"])? 'EditorGridPanel':'GridPanel')?>({
        id:'button-grid',
        ds: store,
        clicksToEdit:2,
