...
class ADODB_oci8 extends ADOConnection {
	...
	// Соответствие типов данных Оракла и внутреннего представления
	var $TypesHash=array(
		"NUMBER"=>"INT",
		"VARCHAR2"=>"VARCHAR",
		"CLOB"=>"TEXT",
	...
	global $ADODB_FETCH_MODE;
		...
		while (!$rs->EOF) { //print_r($rs->fields);
			$fld = new ADOFieldObject();
	   		...
	   		$fld->type = (isset($TypesHash[$rs->fields[1]])? $TypesHash[$rs->fields[1]]:$rs->fields[1]);
...
"select constraint_name,r_owner,r_constraint_name 
	...
	function _insertid($tabname="")
	{   
		if(!$tabname) {echo "No Table Name!";return 0;}
		return ADOConnection::GetOne('select '.$tabname.'_seq.currval from dual');
