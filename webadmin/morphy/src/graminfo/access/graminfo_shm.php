...
class phpMorphy_Graminfo_Shm extends phpMorphy_Graminfo {
	...
	function readAncodes($info) {
		...
		return explode("\x0", shmop_read($shm, $info['offset'] + 10 + $info['all_size'], $info['ancodes_size'] - 1));
	}
	
	function readFlexiaData($info, $onlyBase) {
		$shm = $this->resource;
		 
		return explode("\x0", shmop_read($shm, $info['offset'] + 10, ($onlyBase ? $info['base_size'] : $info['all_size']) - 1));
	}
	...
	function readAllGramInfoOffsets() {
		...
		for($offset = 0x100, $i = 0, $c = $this->header['flex_count']; $i < $c; $i++) {
			...
			$pad_len = ord(shmop_read($shm, $offset + $flexia_size, 1));
