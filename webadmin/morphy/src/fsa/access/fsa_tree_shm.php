...
class phpMorphy_Fsa_Tree_Shm extends phpMorphy_Fsa {
	function walk($trans, $word, $readAnnot = true) {
		$shm = $this->resource; $fsa_start = $this->fsa_start;
		...
		for($i = 0, $c = strlen($word); $i < $c; $i++) {
			...
			// tree version
			$result = true;
			$start_offset = $fsa_start + (($trans >> 11) << 2);
			
			// read first trans in state
			 
			list(, $trans) = unpack('V', shmop_read($shm, $start_offset, 4));
			...
			if($result) {
				...
				for($idx = 1, $j = 0; ; $j++) {
					...
					list(, $trans) = unpack('V', shmop_read($shm, $start_offset + (($idx - 1) << 2), 4));
				}
			}
		...
		if($i >= $c) {
			...
			if($readAnnot) {
				...
				list(, $trans) = unpack('V', shmop_read($shm, $fsa_start + (($trans >> 11) << 2), 4));
	...
	function collect($startNode, $callback, $readAnnot = true, $path = '') {
		...
		$state = $this->readState(($startNode) >> 11);
	...
	function readState($index) {
		...
		list(, $trans) = unpack('V', shmop_read($shm, $offset, 4));
	...
	protected function readRootTrans() {
		...
		list(, $trans) = unpack('V', shmop_read($shm, $fsa_start + 0, 4));
	...
	protected function readAlphabet() {
		...
		return shmop_read($shm, $this->header['alphabet_offset'], $this->header['alphabet_size']);
	}
	...
	function getAnnot($trans) {
		...
		$offset =
			...
			((($trans & 0xFF) << 21) | $trans >> 11);
		
		 
		$len = ord(shmop_read($shm, $offset, 1));
		
		if($len) {
			$annot = shmop_read($shm, $offset + 1, $len);
		} else {
			$annot = null;
		}
