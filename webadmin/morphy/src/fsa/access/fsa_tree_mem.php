...
class phpMorphy_Fsa_Tree_Mem extends phpMorphy_Fsa {
	function walk($trans, $word, $readAnnot = true) {
		...
		for($i = 0, $c = strlen($word); $i < $c; $i++) {
			...
			if($result) {
				...
				for($idx = 1, $j = 0; ; $j++) {
					...
					list(, $trans) = unpack('V', substr($mem, $start_offset + (($idx - 1) << 2), 4));
				}
			}
		...
		if($i >= $c) {
			...
			if($readAnnot) {
				...
				list(, $trans) = unpack('V', substr($mem, $fsa_start + (($trans >> 11) << 2), 4));
	...
	function readState($index) {
		...
		list(, $trans) = unpack('V', substr($mem, $offset, 4));
	...
	protected function readRootTrans() {
		...
		list(, $trans) = unpack('V', substr($mem, $fsa_start + 0, 4));
