...
class phpMorphy_Fsa_Sparse_Shm extends phpMorphy_Fsa {
	function walk($trans, $word, $readAnnot = true) {
		...
		for($i = 0, $c = strlen($word); $i < $c; $i++) {
			...
			list(, $trans) = unpack('V', shmop_read($shm, $fsa_start + ((($trans >> 10) + $char + 1) << 2), 4));
		...
		if($i >= $c) {
			...
			if($readAnnot) {
				...
				list(, $trans) = unpack('V', shmop_read($shm, $fsa_start + (($trans >> 10) << 2), 4));
	...
	function readState($index) {
		...
		foreach($this->getAlphabetNum() as $char) {
			...
			list(, $trans) = unpack('V', shmop_read($shm, $start_offset + (($char) << 2), 4));
	...
	protected function readRootTrans() {
		...
		list(, $trans) = unpack('V', shmop_read($shm, $fsa_start + 4, 4));
