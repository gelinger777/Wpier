...
<head>
  ...
  <?=(isset($_CONFIG["VENDOR_CHANEL"]) && $_CONFIG["VENDOR_CHANEL"]? '<script type="text/javascript" src="vendor/rsschanel.js"></script>':'')?>
  ...
  <script type="text/javascript">
    ...
    MENU_STYLES=null;
    _DESKTOP_BACKGROUND_='<?=($ADMIN_WALLPAPER? $ADMIN_WALLPAPER:"")?>';
    STORE_LOCATIONS=[];
    CAN_CHMOD=false;
    _TREE_ACCESS_=0;
    _ADMIN_SEARCH='<?=(isset($_CONFIG["SERCH_IN_ADMIN"])? $_CONFIG["SERCH_IN_ADMIN"]:'')?>';
    _LOCALE_DIR='location/<?=$_CONFIG["ADMIN_LOCATION"]?>/';
    _HELP_STATUS=<?=(isset($_CONFIG["HELP"]) && $_CONFIG["HELP"]? "true":"false")?>;
    _VENDOR_CHANEL='<?=(isset($_CONFIG["VENDOR_CHANEL"]) && $_CONFIG["VENDOR_CHANEL"]? $_CONFIG["VENDOR_CHANEL"]:'')?>';
    _URI_PREVIEW_TPL='<?=(isset($_CONFIG["URI_PREVIEW_TPL"]) && $_CONFIG["URI_PREVIEW_TPL"]? $_CONFIG["URI_PREVIEW_TPL"]:'/%id%.html?prev=%access%')?>';
    <?
    ...
    Ext.onReady(function(){
       ...
       //var dz2 = new Ext.dd.DropZone('desktop-view', {ddGroup:'desktop'});
       <?
       //include "readprogramms.php";
	   ...
	   for(var i in PLUGINS.main) eval('PLUGINS.main.'+i+'.init()');
