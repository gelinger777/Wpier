...
IO=new function(){
  ...
  // Функция связывания файлов с программами
  this.associate=function(f) {
    var i=f.length-1;
    while(i>0 && f.charAt(i)!='.') i--;
    if(i>0) {
      i=f.substr(i+1);
      i=i.toLowerCase();
      eval("i=this.file2prog."+i);
      if(i!=null && i!="undefined") i(f);
      else return false;
    }
	return true;
  }

  this.download=function(f) {
	f=f.split(':');
	if(STORE_LOCATIONS[f[0]]!=null) {
		window.location=STORE_LOCATIONS[f[0]][0]+'?getfile='+f[1];
	}
  }
      ...
      if(STORE_LOCATIONS[fn[0]][2]) { // Пассивный режим, постим на сторе
        ...
        Ext.Ajax.request({
           ...
           success: function(response) {
             if(response.responseText!="") handler(true); 
             else handler(false);              
           },
           params: {
             savevalue:escape(content),
             fn:fn[1],
             sendto:STORE_LOCATIONS[fn[0]][0]
           }
        });
         ...
         Ext.Ajax.request({
           ...
           success: function(response) {
             if(response.responseText!="") {
               ...
               s+="&ret=IO.RunHandler&oid="+IO.handlers.length;
               IO.handlers[IO.handlers.length]=handler;
               IO.sendScriptQuery(s);
             } else handler(false);              
           },
           params: { savevalue: escape(content) }
         }); 
...
AJAX=new function() {
 ...
 this.sendScriptQuery=function(url,nodel) {
   var o = new Date();
   ...
   o.setAttribute("type", "text/javascript");
   o.setAttribute("src", url);		
   if(nodel==null || !nodel) setTimeout(function(){document.body.removeChild(o);}, 15000);
 } 
 
 this.get=function(mod,param,mode) {
   var x;
 ...
 //prompt('',mod+'.php?'+q.join('&'))  
   return this.loadHTML(mod+'.php?'+q.join('&'),mode);
 }
 
 this.post=function(mod,get,post,mode) {
   var x;
   var q='';
   if(mode==null) mode=false;
   if(get!=null) {
   ...
   return this.loadHTML(mod,mode,null,'POST',q);
 } 
