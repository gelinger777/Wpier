function iCommanderClass(obj) {
  ...
  this.conf={
    ...
    singleSelect:false,
    handler:function(f) {
      if(!IO.associate(f)) IO.download(f); // если с файлом не связано приложений, качаем его
    },
    mask:[['*.*','All formats (*.*)']]
  };
  if(obj!=null) {
...
//return ar;
	...
	var j,l,x,out=[];     
    var re=this.currmask.replace(/\*/g,'[0-9a-zа-я~\\s_-]*');
    re=re.replace(/\./,'\\.');
    re=re.split(',')
    var rg=[];
...
//alert(out.join(';'));    
  ...
  this.ExeFile=function(fn,grid,ctrl) {
    ...
    if(this.filename!=null) {
      ...
      if(s!='') {
        ...
        if(ctrl) IO.download(this.FileName);
		else this.conf.handler(this.FileName);        
      }
      this.Window.close();
    } else {
      fn=grid.currentStore+':'+fn;
      if(ctrl) IO.download(fn);
	  else this.conf.handler(fn)
    }      
  }
  ...
  this.GoToDir=function(dir,grid,ref) {
    ...
    dir=i+(i.indexOf('?')==-1? '?':'&')+'store='+grid.currentStore+'&grid='+grid.id+'&dir='+dir; 
    //prompt('',dir);
    IO.sendScriptQuery(dir);
  }
      ...
      if(this.conf.rpanelmode=='preview' && sm.selections.length==1) {
        ...
        if(sm.selections.items[0].data.t!='dir') {
          ...
          this.PreviewTimeout=window.setTimeout(function() {
            IO.sendScriptQuery(STORE_LOCATIONS[sm.grid.currentStore][0]+'?preview='+sm.grid.currentPath+sm.selections.items[0].data.f.replace('|*e*|','.')+'&size='+(o.rpanel.getInnerWidth()-30)+'x'+(o.rpanel.getInnerWidth()-30)+'&ret=IO.ShowPreview&oid='+o.id+'PP');
          ...
          rowdblclick :function(g,i,e) {
            ...
            if(ob.data!=null && ob.data.f!=null) {
              ...
              if(ob.data.e=='\t[DIR]') {
                ...
                o.ExeFile(this.currentPath+ob.data.f.replace('|*e*|','.'),this,e.ctrlKey); 
              }
            }
          },
          click:function(e) {
  ...
  this.SaveStore=function() {
    ...
    try {
      AJAX.post('ajaxfunc',{},{stores:escape('['+s.join(',')+']')});
    } catch(e) {}
  ...
  this.ChangeStore=function(n,grid) {
    ...
    if(STORE_LOCATIONS[n][3]==null) {
      ...
      url+=(url.indexOf('?')==-1? '?':'&')+'store='+n+'&grid='+grid.id;
      //prompt(url);      
  ...
  this.MkWin=function() {
    ...
    this.Window=new Ext.Window({
      ...
      id:'iFileManager-'+o.id, // Поправлено 16.03.09
      width:o.conf.width,
